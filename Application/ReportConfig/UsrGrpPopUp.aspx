<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="UsrGrpPopUp.aspx.vb" Inherits="ReportConfig.UsrGrpPopUp" %>
...
<head>
    ...
    <script language="javascript" src="../JScript/Common2.js"></script>
       ...
       <script language="javascript">
        ...
        {
            pageURL ="UgpDet.aspx?UgId=" + UserGroupId
...
//        }
        ...
        var KfLableVal=document.all("KdDes"+ RowCnt).innerHTML
		var KdIdVal=document.all("KdId"+ RowCnt).innerText
		var KdDesVal=document.all("KdDes"+ RowCnt).innerText
...
//        document.getElementById("RBVal").value=KdIdVal
        ...
        if(<%=TT%>==0)
        {
            opener.FnAssignUsrGrpVal(KdIdVal,KdDesVal,<%=icnt%>,<%=jcnt%>)
        }
        else
        {
            opener.FnAssignMailUsrGrpVal(KdIdVal,KdDesVal,<%=icnt%>,<%=jcnt%>)
        }
	...
	function fnCrCancel(){
	    if(<%=TT%>==0)
	    {
	        opener.FnAssignUsrGrpVal('','','','')
	    }
	    else
	    {
	        opener.FnAssignMailUsrGrpVal('','','','')
	    }
