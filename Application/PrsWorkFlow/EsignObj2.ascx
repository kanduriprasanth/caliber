<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="EsignObj2.ascx.vb" Inherits="PrsWorkFlow.EsignObj2" %>
...
var ObjPrefix =document.all("EsignObjId").value;
var SpecificAct=''
var AppAction
var EsignFor 
var RowCount
...
AppAction='Re-test';
 ...
 function fnCallESign(EsignType,RowCnt){	
 ...
 RowCount=RowCnt
	...
	if( ESEnabled=="1" && !IsValidFlag){	
		...
		if((document.all(ESWindowPathId).value).length > 5)
		var pageURL = document.all(ESWindowPathId).value
		else
...
document.all(UsId).value=u
document.all(Pwsid).value=p
...
function fnIsESignValid(returnStatus){
	...
	if(returnStatus){
		...
		for (i = 0; i < document.forms(0).elements.length; i++)
	        {	  
                    if (EsignFor == "1")  // At Batch Status Registration
                    { 
                          Ajax_SaveInitiatorDetails(RowCount);
                          break;
                    }

                    if (EsignFor == "2" || EsignFor == "3" || EsignFor == "4")  // At Extended Target Registration Approval Approve/Return/Reject
                    { 
                       Ajax_AppDetails(RowCount,EsignFor);
                        break;
                    }
...
function fnActivity(){
	...
	var ScreenTitle=document.all(ScreenTitleId).value
	var TrnTypeId=ObjPrefix + "_TrnType"
	var Activity
     
	var TrnTypVal=document.all(TrnTypeId).value
	Activity=''
