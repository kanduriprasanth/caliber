<%@ Control Language="vb" AutoEventWireup="false" Codebehind="EsignObj1.ascx.vb" Inherits="PrsWorkFlow.EsignObj1" %>
...
function fnIsESignValid(returnStatus){
	...
	if(returnStatus){
		...
		for(i=0;i<document.forms(0).elements.length;i++)
	        {	
                    if (EsignFor == "1")  // At Acrion Plan  Modification Initaiion
                    { 
                      AjaxModifyActPlan(RowCount);
                       break;
                    }
                    else if (EsignFor == "2" || EsignFor == "4" )  // At Action Plan Modfication Approval/ Reject
                     {
                        AjaxApproveActPlan()
                        break;
                      }
                    
                    else if (EsignFor == "3") // At Acrion Plan  Initiaion
                    {
                        ButtonName=''
...
function fnActivity(){
  ...
  if(TrnTypVal=="8"){
	...
	document.all(ESActivityID).value=Activity;
	return(Activity);
