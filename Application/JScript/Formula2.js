...
//############################  FORMULA VALIDATION  ####################
function Formula(objName,formName,symbolList,src,args)
{
	...
	var str2 = new String(str)
	   ...
	   var regEx1 = new RegExp ('tan', 'gi') ;

                 str2= str2.replace(regEx, '')
		 str = str2.replace(regEx1, '')
	...
	if(flag==0){
		var FStatus = FormulaSymbolTest(objName,formName,symbolList,src,args)
		if(FStatus==false)
		return(false)
	}
	 return(true);
...
function FormulaSymbolTest(objName,formName,symbolList,src,args)
{
...
//######  Passlimits function for Quantitative and MultiQuantitative Tests #####
	function PassLimitsValidation(num,Type,formName,ObjPrefix,src,args)
	{
		var form2 = formName
		for(i=1;i<=num;i++)
		{
	...
	function Formula2(objName,formName,symbolList,src,args)
       {
	var str = document.all(formName).all(objName).value
	var tempStr=""
	if(str == "")
	...
	if(flag==1){
		...
		flag=0;
                document.all(formName).all(objName).focus()
		document.all(formName).all(objName).select()
		return(false)
	}
