...
//  For Stand Text popup
  ...
  function fnOpenStandardPopUp(BaseId,CtrlID)
  {
    //SetPopDimensions()
    var PWidth=550
    var PHeight=420
    ...
    var pageURL = "../IssueLogin/StdtxtPopupPage.aspx?BaseID="+ BaseId + "&CtrlId="+CtrlID
    var features = "height=" + PHeight + ",width=" + PWidth + ",toolbar=no,resizable=no,menubar=no,status=no,scrollbars=yes,top=" + ScreenTop + ",left=" + ScreenLeft
    SpWinObj = window.open(pageURL,'SpecCodes',features)
    if(SpWinObj.opener == null) SpWinObj.opener=self;
    SpWinObj.focus();
  ...
  function SetStandardTxtValue(BaseId,CtrlID,Standardtxt) {
   if(Standardtxt!=''){
   ...
   // if (FunctId !=0) QAMS_SpecialValidations(FunctId,BaseID,CtrlId,document.getElementById("IssueID").value)
    ...
    {     
        ...
        if (BaseId =='FLD_61' && CtrlId==30)  {LotID ='FLD_60_29'}
         ...
         var pageURL = "../IssueLogin/MultirefObjPopup.aspx?BaseID="+ BaseId + "&CtrlId="+CtrlId + "&StatusFlag=1&ObjectType=" + ObjectType  + "&SheetId=" + SheetId + "&FldCaption=" + FldCaption + "&IsuBaseID=" + IsuBaseID + '&LotID=' + LotID
         var features = "height=" + PHeight + ",width=" + PWidth + ",toolbar=no,resizable=no,menubar=no,status=no,scrollbars=yes,top=" + ScreenTop + ",left=" + ScreenLeft
         SpWinObj = window.open(pageURL,'SpecCodes',features)
         if(SpWinObj.opener == null) SpWinObj.opener=self;
         SpWinObj.focus();
    ...
    function WorkAllUserGrpPopup(BaseId,CtrlID,IsuLogid,IsuTypeId,CusValFunId)
     {
      //debugger
        ...
        if (BaseId=='FLD_102')  // Deviation
        {  
          IHodId = 'FLD_101' + '_' + CtrlID
        ...
        if (BaseId=='FLD_90')  // Permanent And Temporary Change Control Process
        {  
          IHodId = 'FLD_89' + '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
       ...
       // For Handling Additional Departments Selected not to come in Other Departments During Return
        ...
        if (BaseId=='FLD_101')  // Deviation
        {  
          IHodId = 'FLD_102' + '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
        ...
        if (BaseId=='FLD_89')  // Permanent And Temporary Change Control Process
        {  
          IHodId = 'FLD_90' + '_' + CtrlID
          var IHODSelDept= document.getElementById(IHodId).value
          if (IHODSelDept !='')
          {
            var IHODSelDeptArr = new Array()
   ...
   function funRedRefIsuewList1(ObjectType,ObjectCode,ObjectID,IsuBaseID)
   	{
        ...
        var features = "height=600,width=700,toolbar=no,resizable=no,menubar=no,status=no,scrollbars=no,top=250,left=150"
    ...
    // **** DMS File popup ********//
           ...
           function CalEditableDMSPopup(RepositoyTypeId,SourceTypeId,SourceId,PlantID,FixedType,IsuTransCode,UParam)
            ...
            function Dmspreview(DocId)
            {
                    ...
                    SpWinObj = window.open(pageURL,'TemplatePreview',"height=700,width=900,toolbar=no,resizable=true,menubar=no,status=no,scrollbars=yes,top=90,left=100");
                    if(SpWinObj.opener == null) SpWinObj.opener=self;
                    SpWinObj.focus();
            }
...
//Incharge group Details
 ...
 //Related to Batch Status 
    ...
    function Fun_BatchSelPreview(BaseId,CtrlId,Type,IsuBaseID,ReqType,WorkItemId)
    {
        var PWidth=800
        var PHeight=600
        var ScreenLeft = parseInt(window.screen.width-PWidth) /2;
        ...
        var pageURL = "../PrsWorkFlow/BatchStatusPreview.aspx?IssuLogId="+ IsuBaseID + "&Type=" + Type + "&BaseID="+ BaseId + "&CtrlId=" + CtrlId + "&ReqType=" + ReqType + "&WorkItemId=" + WorkItemId
        var features = "height=" + PHeight + ",width=" + PWidth + ",toolbar=no,resizable=no,menubar=no,status=no,scrollbars=yes,top=" + ScreenTop + ",left=" + ScreenLeft
        SpWinObj = window.open(pageURL,'SpecCodes',features)
        if(SpWinObj.opener == null) SpWinObj.opener=self;
        SpWinObj.focus();
