...
//Others Field  -- CPA Login
var OthersTxtID = 'FLD_15_5'
var OthersTxtIDArr = new Array()
OthersTxtIDArr = OthersTxtID.split('_')
            ...
            AddCutomValidator("", DevOthersFldIdAry[1], DevOthersFldIdAry[2])
             //END Others
              ...
              if (EventRelTo == "Product") {
                  ...
                  AddCutomValidator("Enter Value For: Batch No(s)", DevBatchFldIdAry[1],DevBatchFldIdAry[2])
        ...
        function FUN_CC_RA_DECISION (BaseId,CtrlId)
        {
          ...
          document.getElementById("FLDLBL_" + BaseId.substring(0,6) + RemarksID + "_11").className = "HideRow";
            ...
            if (document.getElementById(BaseId + "_" + CtrlId + "_1").checked==true)
            {
               ...
               // fun_CC_RA_CheckPlansStatus(document.getElementById("IssueLogIdTXt").value,ExpCtrlID,idStr)
            }   
            setExpMessage(ExpMessage,ExpCtrlID,idStr)
        }
       ...
       // alert(IssueID)
          ...
          // if (QADecision== 'Not Applicable')
           ...
           if (QADecision == 'Mandatory') {
               ...
               var CA_Text = document.getElementById("1_COL1113009005_9").value
               var PA_Text = document.getElementById("1_COL1113009007_20").value
               var OA_Text = document.getElementById("1_COL1113009009_52").value
...
//               AddCutomValidator("Select Value For: Origin of CAPA", 111, 81)
              ...
              for (i = 0; i < CAPAOrignValAry.length; i++) {
                  ...
                  if (CAPAOrignValAry[i] == "Others") {
                      isOthers =1
                      AddCutomValidator("Enter Value For: Others", OthersTxtIDArr[1], OthersTxtIDArr[2])
                      document.getElementById("FLDLBL_" + OthersTxtIDArr[1] + "_" + OthersTxtIDArr[2]).className = "MainTd";
                  }
              } 
              if(isOthers==0)
               {
               document.getElementById("FLDLBL_" + OthersTxtIDArr[1] + "_" + OthersTxtIDArr[2]).className = "HideRow";
               }
          ...
          // **** Custom Field Validations for EForm at OnSubmit******//
            ...
            function CustValidations(SrcType)
            {
                    var ExpMsg=""
                     ...
                     Fun_Check_BatchSelection(SrcType)
                    var CusValFunIdLst =document.getElementById("CustomValidFunTxt").value.substring(1)
                    var CusValFldIdLst =document.getElementById("CunstomValIdsTxt").value.substring(3)
                    var CusValFunIdArr =new Array()
                    var CusValFldIdArr =new Array()
...
//            var LocationDDl=Ucodelevel0
 ...
 // New Functions 
         ...
         // Fun_Check_BatchSelection
                ...
                function Fun_Check_BatchSelection(SrcType)
                {
                    ...
                    if(SrcType ==1) // At  issue Login Approval Cycle
                    {
                      ...
                      if (BatchSelObj != null  && BatchAuthObj != null)
                      {
                          ExpCtrlID=BatchAutID 
                          idStr=BatchAutID
                          
                              if ( (BatchSelObj.value != '') &&  (BatchAuthObj.value == '') && (document.getElementById("BTN_" + BatchAutIDArr[0] + '_' + BatchAutIDArr[1]).disabled == false))
                              {
                                  var r = confirm("Decision On Selected Batch(s) Is Pending  \n Still To Continue Click Ok ");
                                  if (r != true)  { ExpMessage="Selecct Value For: Batch Status Authorization " } ;
                                   
                              }
                            
                              if ( (BatchSelObj.value != '') && (BatchAuthObj.value != '') && (document.getElementById("BTN_" + BatchAutIDArr[0] + '_'  + BatchAutIDArr[1]).disabled == false))
                              {
                                    BatchSelvalArr=BatchSelObj.value.split(',')                                
                    ...
                    if (xmlhttp.readystate == 4)
                     {
                        ...
                        if (Time >0)
                           {
                              var ExpMessage="Select Value For: Batch Status Authorization"
                              setExpMessage(ExpMessage, FldID, FldID);
                           }
                     }
                }       
  ...
  function uploadstatsap(BaseID) {
        ...
        function fun_CC_RA_CheckPlansStatus(IssuelogId,ExpCtrlID,idStr) {
            ...
            xmlhttp.onreadystatechange = function() {
                if (xmlhttp.readystate == 4) {debugger
                    ...
                    RtnStr = xmlhttp.ResponseText
                        ...
                        if (RtnStr>0) {
                          setExpMessage("Some Proposed Plans Are In Active, Until Close  Those Plan You Can Not Submit The Decision ",ExpCtrlID,idStr)
                        }
                    xmlhttp = null;
                }
            }

            var url="../PrsWorkFlow/AjaxReadPlanStatus.aspx?IssuelogId=" + IssuelogId  + '&Type=4'
            xmlhttp.open("POST", url, true);
            xmlhttp.send(null);
        }