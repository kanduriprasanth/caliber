<%@ Control Language="vb" AutoEventWireup="false" Codebehind="SearchCtrl.ascx.vb" Inherits="TestEFormExecution.SearchCtrl" %>
...
<table class="SubTable" id="Table1" style="WIDTH: 100%" cellSpacing="1" cellPadding="0"
	...
	<tr>
		...
		<SELECT id="SearchTypeDD" onchange="DisplayFilterTab()" name="SearchTypeDD">
			...
			<%--	<option value="2">  </option> --%>
			    <option value="1"> <%=HttpContext.Current.Application("ENB043_" & "0")%> </option>
	...
	<tr class="HideRow" id="SrchTr2">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
				<td class="MainTD">Object Name Like &nbsp;<input class="TxtCls" id="SrchTxt2" maxLength="25"  style="WIDTH: 200px" accessKey="D" type="text"
	...
	<tr class="HideRow" id="SrchTr1">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
					<td class="MainTD">&nbsp;<%=HttpContext.Current.Application("ENB043_" & "0")%>  
	...
	<tr class="HideRow">
		  <input type="hidden" value="SrchTxt1,SrchTxt2,<%=me.UniqueID%><%=UniqeIDSep%>FromDate<%=UniqeIDSep%>CalDateVal,<%=me.UniqueID%><%=UniqeIDSep%>ToDate<%=UniqeIDSep%>CalDateVal"
                name="SrchObjIds">
   ...
   var datval,curDateVal
	...
	SrchObjIdAry=SrchObjIdVal.split(',')
		...
		if((SearchTxtVal!='')){ // reset values on page refresh /re-load
			...
			document.forms(0).all(SrchObjIdAry[2]).value=SearchTxtAry[3]
			datval=SearchTxtAry[2]
