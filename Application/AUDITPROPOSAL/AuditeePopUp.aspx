<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="AuditeePopUp.aspx.vb" Inherits="AUDITPROPOSAL.AuditeePopUp" %>
...
<body ms_positioning="GridLayout" onload="document.getElementById('FindTxt').focus()" onblur="document.getElementById('FindTxt').focus()">
	...
	function fnCrCancel(){
		opener.fnGetAuditeeVal('','','',<%=DeptCnt%>,<%=PrcsCnt%>)
		window.self.close()
	}

	function fnCrGetAuditeeCode(RowCnt)
	{ 
...
//		var KdCodeVal=document.all("KdCODE"+ RowCnt).innerText
        ...
        var KdCodeVal =KdCodeVal.join(",")
		document.getElementById("RBVal").value=KdIdVal
		opener.fnGetAuditeeVal(KdIdVal,KdCodeVal,KfLableVal,DeptCnt,PrcsCnt)
	}
	...
	function fnCrCloseWin(){
		...
		document.getElementById("RBVal").value=KdIdVal
	      ...
	      {
             if(<%=Request.QueryString("AdtType")%>==3)
	        {
	            alert("Select At Least One Authorized User")
	        }
	        else
	        {
	            alert("Select At Least One Auditee")
	        }   
            return(false);
          }    
		window.self.close()
