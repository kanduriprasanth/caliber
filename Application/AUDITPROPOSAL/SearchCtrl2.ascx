...
<%@ Control Language="vb" AutoEventWireup="false" Codebehind="SearchCtrl2.ascx.vb" Inherits="AUDITPROPOSAL.SearchCtrl2" %>
		...
		<SELECT id="SearchTypeDD" onchange="DisplayFilterTab()" name="SearchTypeDD">
			<option value="0" selected>Top 250 Records</option>
			   ...
			   <%-- <option value="1"> Header Code</option>--%>
				<option value="2"> Initiated Date</option>
			</SELECT>
		</td>
	</tr>
...
<%--	
	...
	<tr class="HideRow" id="SrchTr1">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
				<td class="MainTD">Audit Code Like &nbsp;<input class="TxtCls" id="SrchTxt1" style="WIDTH: 200px" accessKey="D" type="text"
							name="SrchTxt1"  maxlength=25  value='%'></td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr class="HideRow">
		<td class="MainTd" colSpan="2"><input type="hidden" value="SrchTxt1,SearchCtrl1<%=UniqeIDSep%>FromDate<%=UniqeIDSep%>CalDateVal,SearchCtrl1<%=UniqeIDSep%>ToDate<%=UniqeIDSep%>CalDateVal" name="SrchObjIds">	
		...
		if((SearchTxtVal!='')){ // reset values on page refresh /re-load
		 ...
		 //	document.forms(0).all(SrchObjIdAry[1]).value=SearchTxtAry[1]
	      ...
	      // if calander dates are null then set control obj dates to current date
	       if(document.forms(0).all(SrchObjIdAry[1]).value==''){
				datval=curDateVal 
				document.forms(0).all(SrchObjIdAry[1]).value=curDateVal
				if(datval!=''){
					var datAry =new Array()
					datAry=datval.split('/')
					obj=document.getElementById("SearchCtrl1_FromDate_CalDateDisplay")
