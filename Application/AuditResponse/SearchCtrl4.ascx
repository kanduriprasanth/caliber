...
<%@ Control Language="vb" AutoEventWireup="false" Codebehind="SearchCtrl4.ascx.vb" Inherits="AuditResponse.SearchCtrl4" %>
<table class="SubTable" id="Table1" style="WIDTH: 100%" cellSpacing="1" cellPadding="0"
	...
	<tr>
		...
		<SELECT id="SearchTypeDD" onchange="DisplayFilterTab()" name="SearchTypeDD">
				...
				<option value="2"> Audit Response Code </option>
			   	<option value="3"> Vendor/ Department/ Audit Agency/Customer </option>
				<option value="4"> Initiated Date</option>
			</SELECT>
		</td>
	</tr>
	<tr class="HideRow" id="SrchTr2">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
				<td class="MainTD">Audit Response Code Like &nbsp;<input class="TxtCls" id="SrchTxt2" style="WIDTH: 200px" accessKey="D" type="text"
							name="SrchTxt2"  maxlength=50  value='%'></td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr class="HideRow" id="SrchTr3">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
					<td class="MainTD">&nbsp;Vendor/ Department/ Audit Agency/Customer Like &nbsp;<input class="TxtCls" id="SrchTxt3" style="WIDTH: 200px" type="text"
							name="SrchTxt3"  maxLength="250" value='%'></td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr class="HideRow">
		<td class="MainTd" colSpan="2"><input type="hidden" value="SrchTxt2,SrchTxt3,SearchCtrl4<%=UniqeIDSep%>FromDate<%=UniqeIDSep%>CalDateVal,SearchCtrl4<%=UniqeIDSep%>ToDate<%=UniqeIDSep%>CalDateVal" name="SrchObjIds">	
...
function ResetSearchState(){
   var datval,curDateVal
    var Obj=document.getElementById("SearchCtrl4_SearchTxt")// get Search Text value
 	SearchTxtVal= Obj.value
 	
 	Obj=document.getElementById("SearchCtrl4_SearchType")//get search type value
	SearchTypeVal=Obj.value
 	Obj=document.getElementById("SearchCtrl4_ToDayDate")// get current date value
	...
	SrchObjIdAry=SrchObjIdVal.split(',')
		...
		if((SearchTxtVal!='')){ // reset values on page refresh /re-load
			...
			if(SearchTxtAry.length==1)
			{
			    return;
			}
			if(SearchTxtAry.length>4)
			{
			    var SrchTxtArr1= new Array();
			    for(var i=1; i<SearchTxtAry.length; i++)
			    {
			        SrchTxtArr1.push(SearchTxtAry[i]);
			    }
			    SearchTxtAry=SrchTxtArr1
			}
			...
			datAry=datval.split('/')
			obj=document.getElementById("SearchCtrl4_FromDate_CalDateDisplay")
			...
			var datAry =new Array()
			datAry=datval.split('/')
			obj=document.getElementById("SearchCtrl4_ToDate_CalDateDisplay")
			GetLocalizedDate(datAry[0], datAry[1], datAry[2],obj)
			}
...
function SetSearchTxt(){
	...
	if(SrchObjIdVal!=''){
		...
		var Obj=document.getElementById("SearchCtrl4_SearchTxt") //set search text with  '$*$' as seperator
		Obj.value=SrchValAry.join('$*$')
		Obj=document.getElementById("SearchCtrl4_SearchType")
