<%@ Control Language="vb" AutoEventWireup="false" Codebehind="EsignObj1.ascx.vb" Inherits="ModificationRequest.EsignObj1"  %>
...
function fnIsESignValid(returnStatus){
	...
	if(returnStatus){
		...
		for(i=0;i<document.forms(0).elements.length;i++)
	        {	  
                    if (EsignFor == "1")  // At Extended Target Registration
                    { 
                      Ajax_RegisterETD(RowCount);



                       break;
                    }
                     if (EsignFor == "5")  // At Extended Target Registration Re-initiation


                    { 
                      Ajax_ReInitETD(RowCount);
                       break;
                    }
                    if (EsignFor == "2" || EsignFor == "3" || EsignFor == "4")  // At Extended Target Registration Approval Approve/Return/Reject

                    { 
                    Ajax_ETDAppDetails(RowCount,EsignFor);
                    break;
                    }
