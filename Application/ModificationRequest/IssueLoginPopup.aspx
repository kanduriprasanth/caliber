<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="IssueLoginPopup.aspx.vb" Inherits="ModificationRequest.IssueLoginPopup" %>
...
<body ms_positioning="GridLayout" onload="document.getElementById('FindTxt').focus()" onblur="document.getElementById('FindTxt').focus()">
	...
	{ 
		...
		var KdCODE=document.getElementById("KdCODE" + RowCnt).innerText
		var ChnageReqStatus=document.getElementById("ChnageReqStatus"+ RowCnt).innerText
		
		var PlanCount=document.getElementById("PlanCount"+ RowCnt).innerText
		document.getElementById("RBVal").value = KdIdVal
		if (ChnageReqStatus=="1" && "<%=ListType%>" == "1")
		{
            alert(" Already A Modification Request Is In-Progress on This Issue  ")
            document.getElementById ("RBID"+ RowCnt).checked=false		
        }
		else if (PlanCount !=0 && "<%=ListType%>" == "2" )
		{
		  alert("'Not Allowed', Action Plan(s) Are In-Progress")
          document.getElementById ("RBID"+ RowCnt).checked=false	
		}
		else
		{
		  opener.GetLoginVals(KdIdVal,KdCODE,KfLableVal)
		}  
	}
	function fnCrCancel(){
		opener.GetLoginVals('','','')
	...
	function fnCrCloseWin(){
	      ...
	      {
            alert("Select At Least One Issue Code")
            return(false);
          }    
