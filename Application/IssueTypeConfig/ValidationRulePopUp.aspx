<%@ Page Language="vb" AutoEventWireup="false" Codebehind="ValidationRulePopUp.aspx.vb" enableEventValidation="false"
    Inherits="IssueTypeConfig.ValidationRulePopUp" %>
...
<head>
    ...
    function AltFnAllowNumVal()
    {
        ...
        {debugger
            ...
            if(event.shiftKey == true){return false}
             ...
             else if((e ==46)||(e ==37)||(e ==39)||(e ==9)||(e ==16)||(e>=96 && e <= 105)) {
    ...
    function FnAllowNumVal()
    {
        ...
        if(event.ctrlLeft == true){return false}
        if(event.shiftKey == true){return false}
         else if((e ==8) || (e ==46)||(e ==37)||(e ==39)||(e ==9)||(e ==16)||(e>=96 && e <= 105)) {}
        else if(e ==32){return false}
        else if(isNaN(String.fromCharCode(e))) //Validates for Key Data
        {return false}
    }
...
<body>
    ...
    <script language="javascript">
    var RowSel =""
    var LoginSelFields='<%=LoginSelFields%>'
    var ClosureSelFields='<%=ClosureSelFields%>'
	...
	{	    
		...
		opener.SetSelFieldVal("<%=Prefix%>",'','','','',GetQueryStr("RowCnt"))
		//To set No.of Validators to 1 
        var FieldIdArr = new Array()
        FieldIdArr = FieldIdLst.split(",");
        for(i=0;i<FieldIdArr.length;i++)
        {
          opener.window.document.getElementById("<%=Prefix%>" + FieldIdArr[i]+ GetQueryStr("RowCnt")).value=""  
        }
		window.self.close()
	}
	...
	function fnCrCloseWin()
	{
	 var prefix1="<%=Prefix%>"
        ...
        opener.SetSelFieldVal(prefix1,FieldIdArr,FieldValArr,ValidationTextLst.substring(4),ValidationValLst.substring(3),GetQueryStr("RowCnt"))
		window.self.close()
	}
	...
	function SetSelectedFields()
	{
        ...
        var prefix="<%=Prefix%>"
        ValArr = opener.window.document.getElementById(prefix+"ValidTypeCapTxt"+RowCnt).value.split(",");
        var STempObj
        var SelVal
        ...
        {
            var prefix="<%=Prefix%>"
            //ValArr = opener.window.document.getElementById(prefix + FieldIdArr[k]+RowCnt).value.split(",");
            ValArr = opener.window.document.getElementById(prefix + FieldIdArr[k]+RowCnt).value.split("#*#");
            for(var j=0;j<ValArr.length;j++)
            {
                document.getElementById(FieldIdArr[k]+j).value = ValArr[j];
                if(document.getElementById("ValDDL"+j).value==6) //if(k==6)
               {
...
function LoadComFealds()
{
    ...
    var prefix="<%=Prefix%>"
    var FieldCnt=<%= RowCnt%>
     var FieldIdArr1 = new Array()
    if(prefix=='L_'){
      FieldIdArr1 = LoginSelFields.split(",");
      }
    else if(prefix=='C_'){
       FieldIdArr1 = ClosureSelFields.split(",");   
      }
     
    var oSelect =document.getElementById("ConToComValTxt"+TCnt)
    var RCnt =0
   for(var k=0;k<FieldIdArr1.length;k++)
   // while(opener.window.document.getElementById(prefix+"FldVAl_"+TCnt))
        {
            if (FieldCnt!=FieldIdArr1[k]) //(FieldCnt!=TCnt) //To avoid same Field to be displayed in ControlToCompare
            {
          
                TVal = opener.window.document.getElementById(prefix+"FldVAl_"+FieldIdArr1[k]).innerText
                TVal2= opener.window.document.getElementById(prefix+"FldID_"+FieldIdArr1[k]).value
                TVal2 += "_" + opener.window.document.getElementById(prefix+"FldCtrlID_"+FieldIdArr1[k]).value 
                ...
                {
                    document.getElementById("ConToComValTxt"+RCnt).appendChild(buildOptions(TVal,TVal2))
                    RCnt+=1;
                }
                
            } //End of FieldCnt!=TCnt
            
            //FieldsLst += "<option value='"+ TVal +"'>"+ TVal +"</option>";
            TCnt+=1;
        }

        document.getElementById("ComFieldsTxt").value =_ComFields; //_ComFields.substring(1);
        document.getElementById("ComFieldIDTxt").value =FieldsLst; //FieldsLst.substring(1);
 ...
 function LoaduserFealds()
{
    ...
    var RCnt1 =0
    var DBType= opener.window.document.getElementById(Prefix+"FldDBType_"+Count).innerText

   for(var k=0;k<FieldIdArr1.length;k++)
    //while(opener.window.document.getElementById(Prefix+"FldVAl_"+TCnt1))
    {    
        if (FieldIdArr1[k] != Count) //(TCnt1 != Count)
        {
                TVal1 =''
                TVal2=''
                if(DBType=="DateTime")
                {
                    if(opener.window.document.getElementById(Prefix+"FldDBType_"+FieldIdArr1[k]).innerText == DBType)
                    { 
                        TVal1 = opener.window.document.getElementById(Prefix+"FldVAl_"+FieldIdArr1[k]).innerText
                        TVal2= opener.window.document.getElementById(Prefix+"FldID_"+FieldIdArr1[k]).value
                        if(TVal1 != "")
                        _ComFields1 += "$@$" + TVal1;
                        _FldIdLst += "$@$" + TVal2;
                ...
                {
                    if(TVal1 != "")
                    document.getElementById("UserCtrlValdTxt"+RCnt1).appendChild(buildOptions(TVal1,TVal2))
                    RCnt1+=1;
                }
          }  
    ...
    document.getElementById("ValdFldTxt").value =_ComFields1;//_ComFields1.substring(1);
    document.getElementById("FldIdTxt").value =_FldIdLst;//_FldIdLst.substring(1);
}
