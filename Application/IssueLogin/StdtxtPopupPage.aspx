<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="StdtxtPopupPage.aspx.vb" Inherits="IssueLogin.StdtxtPopupPage" %>
...
<body ms_positioning="GridLayout" onload="document.getElementById('FindTxt').focus()" onblur="document.getElementById('FindTxt').focus()">
	...
	{  
		var StandardCap= document.all("CaptionDesc"+ RowCnt).innerHTML
		document.getElementById("RBVal").value = document.all("KdId"+ RowCnt).innerText
		opener.SetStandardTxtValue("<%=BaseId%>","<%=CtrlId%>","<%=MaxCount%>",StandardCap)
	}
	function fnCrCancel(){
		opener.SetStandardTxtValue("<%=BaseId%>","<%=CtrlId%>","<%=MaxCount%>",'')
		window.self.close()
	}
