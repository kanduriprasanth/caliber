 ...
 PrinterObj=document.all("CaliberPrintCls1")
   ...
   // paperWidth = 115
	...
	if(PageOrientation==0)
       {
		if(IsWithOutLogo!=1) //If printing with Logo
				PrinterObj.PageSettings_Portrait_A4(paperHeight,paperWidth,"Arial",8,5,6.3,85,40,5,40,85,85,5,21,85,60,0.3)
    ...
    ScreenPaperPrintRatio=parseFloat((paperWidth)/(TotalScreenWidth*standardTxtWidth))
	 ...
	 PrinterObj.PrintPicture(70.0,2,15,6.0);
	 }
	...
	*/
    ...
    //End Print in Footer Area
     
    // Call final Print Routine
    PrinterObj.PrintDoc()
...
*/
			...
			if(PageOrientation==0)
				{ 
				PrinterObj.PrintParagraph(Addr,5,85,'Arial',14,'true',0)
				PrinterObj.PrintParagraph(CompanyAddr,5,85,'Arial',12,'true',0)
				}
 			else
 			{
 			PrinterObj.PrintParagraph(Addr,-35,85,'Arial',14,'true',1)
			PrinterObj.PrintParagraph(CompanyAddr,-35,85,'Arial',12,'true',1)
			}
			...
			CellWidth =  570*ScreenPaperPrintRatio		
			PrinterObj.SetTableColWidth(0,CellWidth)
			//CellWidth=  600*ScreenPaperPrintRatio	
			if(PageOrientation==0)
            CellWidth=  150*ScreenPaperPrintRatio	
            else
            CellWidth=  305*ScreenPaperPrintRatio	+10.3
