<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="ProcessPopUp.aspx.vb" Inherits="SelfInspection.ProcessPopUp" %>


<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<html>
<head>
    <title>Audit Area List</title>
...
<body ms_positioning="GridLayout" onload="document.getElementById('FindTxt').focus()" onblur="document.getElementById('FindTxt').focus()">
    ...
    for(var k=0;k<<%=Cnt%>;k++)
    {
        ...
        opener.fnGetFormProcessVal(KdIdVal,KdCodeVal,KfLableVal,KdDesVal,DeptIDVal,<%=Request.QueryString("iCnt")%>,<%=Request.QueryString("jCnt")%>,document.getElementById("IdsTxt").value)
    }
	
	function fnCrCancel(){
		opener.fnGetFormProcessVal('','','','','',<%=Request.QueryString("iCnt")%>,<%=Request.QueryString("jCnt")%>,'')
		window.self.close()
	}
	...
	function fnCrCloseWin(){
	      ...
	      {
            alert("Select At Least One Audit Area")
            return(false);
          }
		window.self.close()
	}
