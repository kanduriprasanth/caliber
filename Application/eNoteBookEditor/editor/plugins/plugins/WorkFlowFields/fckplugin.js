...
FCKCommands.RegisterCommand( 'WorkFlowFields', new FCKDialogCommand( 'WorkFlowFields', FCKLang.WorkFlowFields, FCKPlugins.Items['WorkFlowFields'].Path + 'WorkFlowFields.html', 400, 350 ) ) ;


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton( 'WorkFlowFields', FCKLang.PlaceholderBtn ) ;
oPlaceholderItem.IconPath = FCKPlugins.Items['WorkFlowFields'].Path + 'workflowfields.png' ;
oPlaceholderItem.Tooltip =FCKLang.WorkFlowFields
FCKToolbarItems.RegisterItem( 'WorkFlowFields', oPlaceholderItem ) ;

// The object used for all WorkFlowFields operations.
var FCKData = new Object() ;

// Add a new WorkFlowFields at the actual selection.
...
// Open the WorkFlowFields dialog on double click.
FCKData.OnDoubleClick = function( span )
{
	if ( span.tagName == 'SPAN' && span._fckplaceholder )
		FCKCommands.GetCommand( 'WorkFlowFields' ).Execute() ;
}
...
// We must process the SPAN tags to replace then with the real resulting value of the WorkFlowFields.
