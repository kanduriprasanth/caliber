...
FCKCommands.RegisterCommand( 'eFromFields', new FCKDialogCommand( 'eFromFields', FCKLang.eFromFields, FCKPlugins.Items['eFromFields'].Path + 'eFromFields.html', 400, 450 ) ) ;


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton( 'eFromFields', FCKLang.PlaceholderBtn ) ;
oPlaceholderItem.IconPath = FCKPlugins.Items['eFromFields'].Path + 'eFromFields.png' ;
oPlaceholderItem.Tooltip =FCKLang.eFromFields
FCKToolbarItems.RegisterItem( 'eFromFields', oPlaceholderItem ) ;

// The object used for all InputFields operations.
var FCKData = new Object() ;

// Add a new InputFields at the actual selection.
...
// Open the InputFields dialog on double click.
FCKData.OnDoubleClick = function( span )
{
	if ( span.tagName == 'SPAN' && span._fckplaceholder )
		FCKCommands.GetCommand( 'eFromFields' ).Execute() ;
}
...
// We must process the SPAN tags to replace then with the real resulting value of the InputFields.
