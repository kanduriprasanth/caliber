...
FCKCommands.RegisterCommand( 'WorkSheetFileds', new FCKDialogCommand( 'WorkSheetFileds', FCKLang.WorkSheetFileds, FCKPlugins.Items['WorkSheetFileds'].Path + 'WorkSheetFileds.html', 400, 400 ) ) ;


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton( 'WorkSheetFileds', FCKLang.PlaceholderBtn ) ;
oPlaceholderItem.IconPath = FCKPlugins.Items['WorkSheetFileds'].Path + 'worksheetfileds.gif' ;
oPlaceholderItem.Tooltip =FCKLang.WorkSheetFileds
FCKToolbarItems.RegisterItem( 'WorkSheetFileds', oPlaceholderItem ) ;

// The object used for all WorkSheetFileds operations.
var FCKData = new Object() ;

// Add a new WorkSheetFileds at the actual selection.
...
// Open the WorkSheetFileds dialog on double click.
FCKData.OnDoubleClick = function( span )
{
	if ( span.tagName == 'SPAN' && span._fckplaceholder )
		FCKCommands.GetCommand( 'WorkSheetFileds' ).Execute() ;
}
...
// We must process the SPAN tags to replace then with the real resulting value of the WorkSheetFileds.
