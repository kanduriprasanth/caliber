...
FCKCommands.RegisterCommand( 'CreateData', new FCKDialogCommand( 'CreateData', FCKLang.CreateData, FCKPlugins.Items['CreateData'].Path + 'fck_Data.html', 400, 350 ) ) ;


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton( 'CreateData', FCKLang.PlaceholderBtn ) ;
oPlaceholderItem.IconPath = FCKPlugins.Items['CreateData'].Path + 'createdata.gif' ;
oPlaceholderItem.Tooltip =FCKLang.CreateData
FCKToolbarItems.RegisterItem( 'CreateData', oPlaceholderItem ) ;

// The object used for all CreateData operations.
var FCKData = new Object() ;

// Add a new CreateData at the actual selection.
...
// Open the CreateData dialog on double click.
FCKData.OnDoubleClick = function( span )
{
	if ( span.tagName == 'SPAN' && span._fckplaceholder )
		FCKCommands.GetCommand( 'CreateData' ).Execute() ;
}
...
// We must process the SPAN tags to replace then with the real resulting value of the CreateData.
