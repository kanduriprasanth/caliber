...
FCKCommands.RegisterCommand('ActionplanFields', new FCKDialogCommand('ActionplanFields', FCKLang.ActionplanFields, FCKPlugins.Items['ActionplanFields'].Path + 'ActionplanFields.htm', 400, 350));


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton('ActionplanFields', FCKLang.PlaceholderBtn);
oPlaceholderItem.IconPath = FCKPlugins.Items['ActionplanFields'].Path + 'ActionplanFields.png';
oPlaceholderItem.Tooltip = FCKLang.ActionplanFields
FCKToolbarItems.RegisterItem('ActionplanFields', oPlaceholderItem);

// The object used for all ActionplanFields operations.
var FCKData = new Object();

// Add a new ActionplanFields at the actual selection.
...
// Open the ActionplanFields dialog on double click.
FCKData.OnDoubleClick = function (span) {
    if (span.tagName == 'SPAN' && span._fckplaceholder)
        FCKCommands.GetCommand('ActionplanFields').Execute();
}
...
// We must process the SPAN tags to replace then with the real resulting value of the ActionplanFields.
