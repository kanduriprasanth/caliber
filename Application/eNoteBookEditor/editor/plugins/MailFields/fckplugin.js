...
FCKCommands.RegisterCommand( 'MailFields', new FCKDialogCommand( 'MailFields', FCKLang.MailFields, FCKPlugins.Items['MailFields'].Path + 'MailFields.html', 400, 350 ) ) ;


// Create the "Placeholder" toolbar button.
var oPlaceholderItem = new FCKToolbarButton( 'MailFields', FCKLang.PlaceholderBtn ) ;
oPlaceholderItem.IconPath = FCKPlugins.Items['MailFields'].Path + 'mailfields.png' ;
oPlaceholderItem.Tooltip =FCKLang.MailFields
FCKToolbarItems.RegisterItem( 'MailFields', oPlaceholderItem ) ;

// The object used for all MailFields operations.
var FCKData = new Object() ;

// Add a new MailFields at the actual selection.
...
// Open the MailFields dialog on double click.
FCKData.OnDoubleClick = function( span )
{
	if ( span.tagName == 'SPAN' && span._fckplaceholder )
		FCKCommands.GetCommand( 'MailFields' ).Execute() ;
}
...
// We must process the SPAN tags to replace then with the real resulting value of the MailFields.
