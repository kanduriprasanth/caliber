<%@ Page Language="vb" AutoEventWireup="false" Codebehind="WorkAllocationAndStatus.aspx.vb" Inherits="CAPAWORKS.WorkAllocationAndStatus" 
...
<head>
	...
	{ 
     ...
     {
       var pageURL = "WordFormatReport.aspx?Type=1&IsuLogID=" + <%=Request.QueryString("IsuLogID") %>  + "&IsuTypeId=" + <%=Request.QueryString("IsuTypeID") %> + "&PrsType=" + <%=prsType%> 
	    var features = "height=600,width=800,toolbar=no,resizable=no,menubar=no,status=no,scrollbars=1,top=250,left=150"
	    SpWinObj = window.open(pageURL,'SpecCodes',features)
	    if(SpWinObj.opener == null) SpWinObj.opener=self;
	    SpWinObj.focus();
...
function Ajax_CloseActionplan(RowCnt)
{
         ...
         var recary = new Array(); 
            ...
            xmlhttp.onreadystatechange=function()
            {
             ...
             var url="CloseActionPlan_Ajax.aspx?PlanId=" +  document.getElementById(""+RowCnt).value + "&Comments=" + document.getElementById(""+RowCnt).value
