<%@ Control Language="vb" AutoEventWireup="false" Codebehind="SearchCtrl4.ascx.vb"
    Inherits="CAPAWORKS.SearchCtrl4" %>
...
<table class="SubTable" id="Table1" style="width: 100%" cellspacing="1" cellpadding="0"
    ...
    <tr>
        <td class="SubHeadTD" colspan="2" align="left">
            ...
            <select id="SearchTypeDD" onchange="DisplayFilterTab()" name="SearchTypeDD">
                ...
                <option value="2">
                     <%=Application("QMS003_" & "0")%> 
                </option>
                <option value="1">
                    <%=Application("ENB032_" & "0")%>
                </option>
                <option value="3">
                     <%=Application("A0186_" & "0")%>
                </option>
                  <%-- <option value="4">
                    Issue Category
                </option>--%>
    ...
    <tr class="HideRow" id="SrchTr4">
        <td class="MainTd" colspan="2">
            <table class="SubTable" style="width: 100%" cellspacing="1" cellpadding="0" align="center"
                ...
                <tr>
                    <td class="MainTD">
                       Issue Category Like &nbsp;<input class="TxtCls" id="SrchTxt3" style="width: 200px" accesskey="D"
                            type="text" maxlength="250" value="%" name="SrchTxt3"></td>
                </tr>
            </table>
        </td>
    </tr>
    <tr class="HideRow">
        <td class="MainTd" colspan="2">
            <input type="hidden" value="SrchTxt1,SrchTxt2,<%=me.UniqueID%><%=UniqeIDSep%>FromDate<%=UniqeIDSep%>CalDateVal,<%=me.UniqueID%><%=UniqeIDSep%>ToDate<%=UniqeIDSep%>CalDateVal,SrchTxt3"
                name="SrchObjIds">
   ...
   var datval,curDateVal
	...
	SrchObjIdAry=SrchObjIdVal.split(',')
		...
		if((SearchTxtVal!='')){ // reset values on page refresh /re-load
			...
			document.forms(0).all(SrchObjIdAry[4]).value=SearchTxtAry[4]  // For Category
