<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="SearchCtrl8.ascx.vb" Inherits="Admin.SearchCtrl8" %>
...
<table class="SubTable" id="Table1" style="WIDTH: 100%" cellSpacing="1" cellPadding="0"
	...
	<tr>
		...
		<SELECT id="SearchTypeDD" onchange="DisplayFilterTab()" name="SearchTypeDD">
			<option value="0" selected>Top 250 Records</option>
				<option value="2"> Caption Name </option>
			   <%-- <option value="1"> Header Code</option>--%>
				<option value="3"> Modified On</option>
			</SELECT>
		</td>
	</tr>
	...
	<tr class="HideRow" id="SrchTr2">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
				<td class="MainTD">Caption Name Like &nbsp;<input class="TxtCls" id="SrchTxt2" style="WIDTH: 200px" accessKey="D" type="text"
	...
	<tr class="HideRow" id="SrchTr3">
		<td class="MainTd" colSpan="2">
			<table class="SubTable" style="WIDTH: 100%" cellSpacing="1" cellPadding="0" align="center"
				border="0">
				<tr>
					<td class="MainTD" >Modified Between&nbsp;&nbsp;&nbsp;&nbsp;<uc1:calibercalender id="FromDate" runat="server"></uc1:calibercalender>
						And
	...
	<tr class="HideRow">
		<td class="MainTd" colSpan="2"><input type="hidden" value="SrchTxt1,SrchTxt2,SearchCtrl8<%=UniqeIDSep%>FromDate<%=UniqeIDSep%>CalDateVal,SearchCtrl8<%=UniqeIDSep%>ToDate<%=UniqeIDSep%>CalDateVal" name="SrchObjIds">	
...
function HideAllSrchTypes(){
    ...
    var ObjId
    for(i=1;i<SrchCnt;i++){
     ObjId="SrchTr" +document.all("SearchTypeDD")(i).value//i
...
function ResetSearchState(){
   var datval,curDateVal
    var Obj=document.getElementById("SearchCtrl8_SearchTxt")// get Search Text value
 	SearchTxtVal= Obj.value
 	
 	Obj=document.getElementById("SearchCtrl8_SearchType")//get search type value
	SearchTypeVal=Obj.value
 	Obj=document.getElementById("SearchCtrl8_ToDayDate")// get current date value
	...
	SrchObjIdAry=SrchObjIdVal.split(',')
		...
		if((SearchTxtVal!='')){ // reset values on page refresh /re-load
			...
			datAry=datval.split('/')
			obj=document.getElementById("SearchCtrl8_FromDate_CalDateDisplay")
			...
			var datAry =new Array()
			datAry=datval.split('/')
			obj=document.getElementById("SearchCtrl8_ToDate_CalDateDisplay")
			GetLocalizedDate(datAry[0], datAry[1], datAry[2],obj)
			}
...
function SetSearchTxt(){
	...
	if(SrchObjIdVal!=''){
		...
		var Obj=document.getElementById("SearchCtrl8_SearchTxt") //set search text with  '$*$' as seperator
		Obj.value=SrchValAry.join('$*$')
		Obj=document.getElementById("SearchCtrl8_SearchType")
