<%@ Control Language="vb" AutoEventWireup="false" Codebehind="ESignWebUC.ascx.vb" Inherits="Admin.ESignWebUC"  %>
...
function fnCallESign(source, arguments){
	...
	var ESEnabled = document.all("ESignWebUC1_ESEnabled").value;
	 
	
	if(Page_IsValid && ESEnabled=="1" && !IsValidFlag){		
	if((document.all("ESignWebUC1_ESWindowPath").value).length > 5)
 	var pageURL = document.all("ESignWebUC1_ESWindowPath").value
		else
		 
		var pageURL = "ESignature.aspx"
...
{
window.parent.parent.navigate('WrongAttempts.aspx','CaliberLIMS');
}
function fnOpStRs()
{
window.parent.navigate('STDReasonsReg.aspx','CaliberLIMS');
}
function fnIsESignValid(returnStatus){
	IsValidFlag = returnStatus;
	if(returnStatus){
		var formName = document.forms(0).name
		var ButtonName = document.all("ESignWebUC1_ESButtonName").value
...
function fnActivity(){
	var formName = document.forms(0).name
	return(document.all("ESignWebUC1_ESActivity").value);
}

function fnSetEsingActivity(strActivity,objButton){
	var formName = document.forms(0).name
	document.all("ESignWebUC1_ESActivity").value = strActivity
	document.all("ESignWebUC1_ESButtonName").value = objButton
}
